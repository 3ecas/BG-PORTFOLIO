...
4. **Encode your whole videos folder in one go.** Type `bash tools/encode.sh `, then drag your videos folder onto the terminal window so its path is filled in, and press Enter:
   ...
   Renaming a master makes it a new project with a new folder, so pick your file names before you encode. Moving a master to another folder is fine: it is still recognised. If you export a new version under the same name into the same folder, it is re-encoded into its existing project folder and its block is marked `// UPDATED`, with a note saying whether anything in `content.js` needs to change.
